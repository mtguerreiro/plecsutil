...
First, let's consider the Buck model we've been using so far, and let's assume we want to compare two controllers: a state feedback and a cascaded controller. Now, we define our controller subsystem in PLECS as shown in :numref:`fig-user-guide-controllers-multiple-controller`.

.. figure:: media/user_guide/controllers/multiple/images/controllers.svg
   :name: fig-user-guide-controllers-multiple-controller
   :scale: 85%
   :align: center
   :alt: Buck model with two controllers.
   
   Buck model with two controllers (:download:`download model <media/user_guide/controllers/multiple/buck_multiple_controllers.plecs>`).

In this case, the model is build with a multiport signal switch that selects the source of the control signal.  This switch is controller by the ``CTL_SEL`` constant, whose value is automatically set by ``plecsutil``. Moreover, a ``CTL_EN`` vector, also generated by ``plecsutil``,  can be used to enable the active controller and disable all others.
...
In the script above, there  are two main differences compared to the single controller case. The first one is that the ``params`` function includes a call to :func:`plecsutil.ui.gen_controllers_params`. This function generates the variables ``CTL_SEL`` and ``CTL_EN`` seen in  :numref:`fig-user-guide-controllers-multiple-controller`, along with some supporting logic. The second difference is that at the end of the script, a ``CONTROLLERS`` dictionary is defined, containing the two controllers of the model. Each entry of this dictionary contains a label for the controller, and a :class:`plecsutil.ui.Controller` object initialized with a ``port`` number, a ``get_gains`` callback, and a ``label`` parameter. The value of ``port`` must match the pattern used in the model. For example, referring to :numref:`fig-user-guide-controllers-multiple-controller`, the state feedback controller is connected to port 1 of the multiport switch, and therefore its ``port`` parameter in ``CONTROLLERS`` is set as 1.
